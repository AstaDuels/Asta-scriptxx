...
-- // THEME (PURPLE DARK)
local BG         = Color3.fromRGB(20, 0, 40)
local CARD_BG    = Color3.fromRGB(30, 0, 50)
local CARD_HOV   = Color3.fromRGB(45, 5, 70)
local BORDER     = Color3.fromRGB(80, 20, 120)
local BORDER2    = Color3.fromRGB(100, 30, 150)
local WHITE      = Color3.fromRGB(255, 255, 255)
local DIM        = Color3.fromRGB(200, 180, 220)
local KB_BG      = Color3.fromRGB(15, 0, 30)
...
local function saveConfig()
    ...
    pcall(function() 
        writefile("AstaBatDesyncConfig.json", HttpService:JSONEncode(cfg)) 
    end)
end

local function loadConfig()
    local hasFile = false
    pcall(function() hasFile = isfile("AstaBatDesyncConfig.json") end)
    if not hasFile then return end
    
    local ok, cfg = pcall(function() 
        return HttpService:JSONDecode(readfile("AstaBatDesyncConfig.json")) 
    end)
...
-- // CLEANUP OLD GUI
for _, name in pairs({"EnvyAutoBatDesyncGUI", "MwvaneNewaBatDesyncGUI", "PhazeAutoBatDesyncGUI", "AstaBatDesyncGUI"}) do
...
local function copyDiscordLink()
    local link = "https://discord.gg/sePX6cw87"
    local success = false
    ...
    if not success then
        pcall(function()
            GuiService:CopyToClipboard(link)
            success = true
        end)
    end
    ...
    if not success then
        pcall(function()
            if HttpService.Clipboard then
                HttpService.Clipboard = link
                success = true
            end
        end)
    end
...
-- // DISCORD POPUP (aparece no canto e some após 2s)
local function showDiscordPopup()
    local copied = copyDiscordLink()
    
    local popupGui = Instance.new("ScreenGui")
    popupGui.Name = "DiscordPopup"
    ...
    label.Text = (copied and "✅ " or "⚠️ ") .. "Asta Bat Desync\ndiscord.gg/sePX6cw87"
...
task.spawn(showDiscordPopup)

-- // GUI PRINCIPAL
local gui = Instance.new("ScreenGui")
gui.Name = "AstaBatDesyncGUI"
...
main.Position = UDim2.new(0.5, -120, 0.5, -40)
...
header.BackgroundColor3 = Color3.fromRGB(15, 0, 30)
...
headerPatch.BackgroundColor3 = Color3.fromRGB(15, 0, 30)
...
pillBg.Size = UDim2.new(0, 36, 0, 19)
pillBg.Position = UDim2.new(1, -46, 0.5, -9.5)
pillBg.BackgroundColor3 = Color3.fromRGB(16, 16, 16)
pillBg.BorderSizePixel = 0
pillBg.ZIndex = 8
Instance.new("UICorner", pillBg).CornerRadius = UDim.new(0, 10)
local pStroke = Instance.new("UIStroke", pillBg)
pStroke.Color = BORDER2
pStroke.Thickness = 1
...
toggleClk.Size = UDim2.new(1, 0, 1, 0)
toggleClk.BackgroundTransparency = 1
toggleClk.Text = ""
toggleClk.ZIndex = 6
toggleClk.MouseButton1Click:Connect(function()
    if State.mobileMode then return end
    State.autoBatToggled = not State.autoBatToggled
    setToggleVisual(State.autoBatToggled)
end)
...
RunService.Heartbeat:Connect(function()
    ...
    if target and target.Character then
        ...
        if root then
            ...
            local targetPos = root.Position + Vector3.new(0, 0.9, 0)
...
-- KEYBIND HANDLER (apenas quando não está em modo mobile)
